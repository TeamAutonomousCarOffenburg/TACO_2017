...
class TfExampleDecoderTest(tf.test.TestCase):
    def _EncodeImage(self, image_tensor, encoding_type='jpeg'):
        with self.test_session():
            if encoding_type == 'jpeg':
                ...
                image_encoded = tf.image.encode_png(
                    tf.constant(image_tensor)).eval()
            else:
        ...
        with self.test_session():
            if encoding_type == 'jpeg':
                image_decoded = tf.image.decode_jpeg(
                    tf.constant(image_encoded)).eval()
            elif encoding_type == 'png':
    ...
    def testDecodePngImage(self):
        ...
        self.assertAllEqual(decoded_png,
                            tensor_dict[fields.InputDataFields.image])
        self.assertEqual('image_id',
                         tensor_dict[fields.InputDataFields.source_id])
    ...
    def testDecodeInstanceSegmentation(self):
        ...
        image_tensor = np.random.randint(
            255, size=(image_height, image_width, 3)).astype(np.uint8)
        encoded_jpeg = self._EncodeImage(image_tensor)

        # Randomly generate instance segmentation masks.
        instance_segmentation = (np.random.randint(
            2, size=(num_instances, image_height,
