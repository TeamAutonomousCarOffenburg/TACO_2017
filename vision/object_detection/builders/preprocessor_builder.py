...
RESIZE_METHOD_MAP = {
    preprocessor_pb2.ResizeImage.AREA:
    tf.image.ResizeMethod.AREA,
    preprocessor_pb2.ResizeImage.BICUBIC:
    tf.image.ResizeMethod.BICUBIC,
    preprocessor_pb2.ResizeImage.BILINEAR:
  ...
  """
    ...
    if step_type == 'random_crop_pad_image':
        ...
        return (preprocessor.random_crop_pad_image, {
            ...
            'min_padded_size_ratio': (min_padded_size_ratio
                                      if min_padded_size_ratio else None),
            'max_padded_size_ratio': (max_padded_size_ratio
                                      if max_padded_size_ratio else None),
    ...
    if step_type == 'ssd_random_crop':
        ...
        if config.operations:
            min_object_covered = [
                op.min_object_covered for op in config.operations
            ]
            aspect_ratio_range = [(op.min_aspect_ratio, op.max_aspect_ratio)
                                  for op in config.operations]
            area_range = [(op.min_area, op.max_area)
                          for op in config.operations]
    ...
    if step_type == 'ssd_random_crop_pad':
        ...
        if config.operations:
            ...
            overlap_thresh = [op.overlap_thresh for op in config.operations]
            random_coef = [op.random_coef for op in config.operations]
            min_padded_size_ratio = [(op.min_padded_size_ratio[0],
                                      op.min_padded_size_ratio[1])
                                     for op in config.operations]
            max_padded_size_ratio = [(op.max_padded_size_ratio[0],
                                      op.max_padded_size_ratio[1])
                                     for op in config.operations]
