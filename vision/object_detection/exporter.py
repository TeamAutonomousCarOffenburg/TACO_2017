...
def _image_tensor_input_placeholder():
    ...
    input_tensor = tf.placeholder(
        dtype=tf.uint8, shape=(None, None, None, 3), name='image_tensor')
...
def _tf_example_input_placeholder():
  ...
  """
    ...
    return (batch_tf_example_placeholder, tf.map_fn(
        decode,
  ...
  """
    ...
    def decode(encoded_image_string_tensor):
        image_tensor = tf.image.decode_image(
            encoded_image_string_tensor, channels=3)
        image_tensor.set_shape((None, None, 3))
        return image_tensor

    return (batch_image_str_placeholder, tf.map_fn(
        decode,
...
def _add_output_tensor_nodes(postprocessed_tensors,
  ...
  """
    ...
    outputs['detection_classes'] = tf.identity(
        classes, name='detection_classes')
    outputs['num_detections'] = tf.identity(
...
def _write_graph_and_checkpoint(inference_graph_def, model_path,
                                input_saver_def, trained_checkpoint_prefix):
