...
def padded_one_hot_encoding(indices, depth, left_pad):
  ...
  """
    ...
    def one_hot_and_pad():
        one_hot = tf.cast(
            tf.one_hot(
                tf.cast(indices, tf.int64), depth, on_value=1, off_value=0),
            tf.float32)
        return tf.pad(one_hot, [[0, 0], [left_pad, 0]], mode='CONSTANT')

    result = tf.cond(
        tf.greater(tf.size(indices), 0), one_hot_and_pad,
...
def dense_to_sparse_boxes(dense_locations, dense_num_boxes, num_classes):
  ...
  """
    ...
    box_locations = tf.slice(dense_locations,
                             tf.constant([0, 0]),
                             tf.stack([num_valid_boxes, 4]))
    tiled_classes = [
        tf.tile([i], tf.expand_dims(dense_num_boxes[i], 0))
...
def indices_to_dense_vector(indices,
  ...
  """
    ...
    return tf.dynamic_stitch([tf.range(size),
                              tf.to_int32(indices)], [zeros, values])
        ...
        for key in tensor_dict:
            ...
            # Needs to reshape into [num_boxes, -1] for tf.gather() to work.
                ...
                valid_dict[key] = tf.reshape(
                    tf.gather(
                        tf.reshape(tensor_dict[key], [num_boxes, -1]),
                        valid_indices), [-1])
...
def retain_groundtruth_with_positive_classes(tensor_dict):
  ...
  """
    ...
    keep_indices = tf.where(
        tf.greater(tensor_dict[fields.InputDataFields.groundtruth_classes], 0))
...
def filter_groundtruth_with_nan_box_coordinates(tensor_dict):
  ...
  """
    ...
    nan_indicator_vector = tf.greater(
        tf.reduce_sum(
            tf.to_int32(tf.is_nan(groundtruth_boxes)), reduction_indices=[1]),
        0)
...
def normalize_to_target(inputs,
  ...
  """
    with tf.variable_scope(scope, 'NormalizeToTarget', [inputs]):
        ...
        lengths = epsilon + tf.sqrt(
            tf.reduce_sum(tf.square(inputs), dim, True))
