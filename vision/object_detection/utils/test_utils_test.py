...
class TestUtilsTest(tf.test.TestCase):
    def test_diagonal_gradient_image(self):
        ...
        expected_first_channel = np.array(
            [[3, 2, 1, 0], [4, 3, 2, 1], [5, 4, 3, 2]], dtype=np.float32)
        self.assertAllEqual(
            np.squeeze(pyramid_image[:, :, 0]), expected_first_channel)

        # Actual test.
        expected_image = np.array(
            [[[3, 30], [2, 20], [1, 10], [0, 0]],
             [[4, 40], [3, 30], [2, 20], [1, 10]], [[5, 50], [4, 40], [3, 30],
                                                    [2, 20]]],
            dtype=np.float32)
    ...
    def test_random_boxes(self):
        ...
        max_height = 3
        max_width = 5
        boxes = test_utils.create_random_boxes(num_boxes, max_height,
                                               max_width)
