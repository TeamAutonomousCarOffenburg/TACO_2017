  ...
  """
    with tf.name_scope(scope, 'PruneSmallBoxes'):
        ...
        is_valid = tf.logical_and(
            tf.greater_equal(width, min_side),
    ...
    with tf.name_scope(scope, 'SortByField'):
        ...
        length_assert = tf.Assert(
            tf.equal(num_boxes, num_entries), [
                'Incorrect field size: actual vs expected.', num_entries,
                num_boxes
            ])
        ...
        with tf.control_dependencies([length_assert]):
            ...
            with tf.device('/cpu:0'):
                _, sorted_indices = tf.nn.top_k(
                    field_to_sort, num_boxes, sorted=True)
...
def filter_field_value_equals(boxlist, field, value, scope=None):
  ...
  """
    with tf.name_scope(scope, 'FilterFieldValueEquals'):
        ...
        gather_index = tf.reshape(
            tf.where(tf.equal(filter_field, value)), [-1])
  ...
  """
    with tf.name_scope(scope, 'ToAbsoluteCoordinates'):
        ...
        if check_range:
            ...
            max_assert = tf.Assert(
                tf.greater_equal(1.01, box_maximum), [
...
def refine_boxes_multi_class(pool_boxes,
  ...
  """
    ...
    for i in range(num_classes):
        ...
        refined_boxes_class = refine_boxes(
            boxes_class, nms_iou_thresh, nms_max_detections, voting_iou_thresh)
