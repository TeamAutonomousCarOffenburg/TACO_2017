...
class TargetAssigner(object):
    ...
    """
        ...
        with tf.control_dependencies([shape_assert]):
            ...
            cls_weights = self._create_classification_weights(
                match, self._positive_class_weight,
                self._negative_class_weight)

            num_anchors = anchors.num_boxes_static()
            if num_anchors is not None:
                reg_targets = self._reset_target_shape(reg_targets,
                                                       num_anchors)
                cls_targets = self._reset_target_shape(cls_targets,
                                                       num_anchors)
                reg_weights = self._reset_target_shape(reg_weights,
                                                       num_anchors)
                cls_weights = self._reset_target_shape(cls_weights,
                                                       num_anchors)
    ...
    """
        matched_indicator = tf.cast(match.matched_column_indicator(),
                                    tf.float32)
        ignore_indicator = tf.cast(match.ignored_column_indicator(),
                                   tf.float32)
...
def batch_assign_targets(target_assigner, anchors_batch, gt_box_batch,
  ...
  """
    ...
    if not all(
            isinstance(anchors, box_list.BoxList)
            for anchors in anchors_batch):
        raise ValueError(
            'anchors_batch must be a BoxList or list of BoxLists.')
    if not (len(anchors_batch) == len(gt_box_batch) ==
    ...
    reg_weights_list = []
    match_list = []
    for anchors, gt_boxes, gt_class_targets in zip(anchors_batch, gt_box_batch,
                                                   gt_class_targets_batch):
