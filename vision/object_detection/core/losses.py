    ...
    """
        ...
        if self._anchorwise_output:
            return tf.reduce_sum(
                per_entry_cross_ent * tf.expand_dims(weights, 2), 2)
...
class HardExampleMiner(object):
  ...
  """
    ...
    """
        ...
        if not len(location_losses) == len(decoded_boxlist_list) == len(
                cls_losses):
            raise ValueError(
        ...
        for ind, detection_boxlist in enumerate(decoded_boxlist_list):
            ...
            mined_location_losses.append(
                tf.reduce_sum(
                    tf.gather(location_losses[ind], selected_indices)))
    ...
    def summarize(self):
        ...
        if self._num_positives_list and self._num_negatives_list:
            ...
            tf.summary.scalar('HardExampleMiner/NumPositives',
                              avg_num_positives)
            tf.summary.scalar('HardExampleMiner/NumNegatives',
                              avg_num_negatives)
    ...
    """
        ...
        max_negatives = tf.maximum(
            ...
            tf.to_int32(
                max_negatives_per_positive * tf.to_float(num_positives)))
