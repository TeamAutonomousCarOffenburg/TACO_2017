...
def _apply_with_random_selector(x, func, num_cases):
  ...
  """
    ...
    return control_flow_ops.merge([
        func(control_flow_ops.switch(x, tf.equal(rand_sel, case))[1], case)
...
def retain_boxes_above_threshold(boxes,
  ...
  """
    with tf.name_scope(
            'RetainBoxesAboveThreshold', values=[boxes, labels, label_scores]):
        indices = tf.where(
  ...
  """
    ...
    with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
        ...
        if boxes is not None:
            boxes = tf.cond(do_a_flip_random, lambda: flip_boxes(boxes),
                            lambda: boxes)
            result.append(boxes)

        # flip masks
        if masks is not None:
            masks = tf.cond(do_a_flip_random, lambda: _flip_masks(masks),
                            lambda: masks)
            result.append(masks)
  ...
  """
    ...
    with tf.name_scope('RandomJitterBoxes', values=[boxes]):
        ...
        distorted_boxes = tf.map_fn(
            lambda x: random_jitter_box(x, ratio, seed),
            boxes,
            dtype=tf.float32)
  ...
  """
    with tf.name_scope('RandomCropImage', values=[image, boxes]):
        ...
        if masks is not None:
            ...
            new_masks = tf.slice(masks_of_boxes_completely_inside_window,
                                 masks_box_begin, masks_box_size)
            result.append(new_masks)

        if keypoints is not None:
            keypoints_of_boxes_inside_window = tf.gather(
                keypoints, inside_window_ids)
            ...
            new_keypoints = keypoint_ops.prune_outside_window(
  ...
  """
    ...
    with tf.name_scope('RandomCropToAspectRatio', values=[image]):
        ...
        def target_height_fn():
            return tf.to_int32(
                tf.round(
                    tf.to_float(orig_height) * orig_aspect_ratio /
                    new_aspect_ratio))
        ...
        def target_width_fn():
            return tf.to_int32(
                tf.round(
                    tf.to_float(orig_width) * new_aspect_ratio /
                    orig_aspect_ratio))
        ...
        offset_height = _random_integer(0, orig_height - target_height + 1,
                                        seed)
        ...
        if masks is not None:
            ...
            new_masks = tf.slice(masks_inside_window, masks_box_begin,
                                 masks_box_size)
            result.append(new_masks)
  ...
  """
    ...
    with tf.name_scope('ResizeToRange', values=[image, min_dimension]):
        ...
        new_image = tf.image.resize_images(
            image, new_size, align_corners=align_corners)
...
def scale_boxes_to_pixel_coordinates(image, boxes, keypoints=None):
  ...
  """
    ...
    if keypoints is not None:
        scaled_keypoints = keypoint_ops.scale(keypoints, image_height,
                                              image_width)
...
def ssd_random_crop(image,
  ...
  """
    ...
    result = _apply_with_random_selector_tuples(
        tuple(
            t for t in (image, boxes, labels, masks, keypoints)
            if t is not None),
...
def ssd_random_crop_fixed_aspect_ratio(
  ...
  """
    ...
    crop_result = ssd_random_crop(
        image, boxes, labels, masks, keypoints, min_object_covered,
...
def preprocess(tensor_dict, preprocess_options, func_arg_map=None):
  ...
  """
    ...
    for option in preprocess_options:
        ...
        arg_names = [
            arg_name for arg_name in arg_names if arg_name is not None
        ]
