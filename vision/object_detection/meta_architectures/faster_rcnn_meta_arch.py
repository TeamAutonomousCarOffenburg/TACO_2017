...
class FasterRCNNFeatureExtractor(object):
    ...
    """
        with tf.variable_scope(scope, values=[proposal_feature_maps]):
            return self._extract_box_classifier_features(
                proposal_feature_maps, scope)
...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    """
        ...
        return (
            _batch_gather_kept_indices(box_encodings),
    ...
    """
        with tf.name_scope('FirstStagePostprocessor'):
            ...
            if self._first_stage_only:
                proposal_boxes, proposal_scores, num_proposals = self._postprocess_rpn(
                    prediction_dict['rpn_box_encodings'], prediction_dict[
                        'rpn_objectness_predictions_with_background'],
                    prediction_dict['anchors'], image_shape)
                return {
        ...
        with tf.name_scope('SecondStagePostprocessor'):
            mask_predictions = prediction_dict.get(
    ...
    def _unpad_proposals_and_sample_box_classifier_batch(
            self, proposal_boxes, proposal_scores, num_proposals,
    ...
    """
        ...
        for (single_image_proposal_boxes, single_image_proposal_scores,
            ...
            sliced_static_shape = tf.TensorShape(
                [tf.Dimension(None), static_shape.dims[-1]])
            single_image_proposal_boxes = tf.slice(
                single_image_proposal_boxes, [0, 0],
                [single_image_num_proposals, -1])
            single_image_proposal_boxes.set_shape(sliced_static_shape)

            single_image_proposal_scores = tf.slice(
                single_image_proposal_scores, [0],
            ...
            sampled_boxlist = self._sample_box_classifier_minibatch(
                single_image_boxlist, single_image_groundtruth_boxlist,
    ...
    def _sample_box_classifier_minibatch(self, proposal_boxlist,
                                         groundtruth_boxlist,
    ...
    """
        (cls_targets, cls_weights,
         _, _, _) = self._detector_target_assigner.assign(
        ...
        sampled_indices = self._second_stage_sampler.subsample(
            tf.cast(cls_weights, tf.bool), self._second_stage_batch_size,
            positive_indicator)
    ...
    """
        ...
        cropped_regions = tf.image.crop_and_resize(
            ...
            get_box_inds(proposal_boxes_normalized), (self._initial_crop_size,
    ...
    """
        ...
        if mask_predictions is not None:
            ...
            mask_predictions_batch = tf.reshape(mask_predictions, [
                -1, self.max_num_proposals, self.num_classes, mask_height,
                mask_width
            ])
        ...
        if mask_predictions is not None:
            detections['detection_masks'] = tf.to_float(
                tf.greater_equal(detections['detection_masks'],
                                 mask_threshold))
        return detections
            ...
            if not self._first_stage_only:
                loss_dict.update(
                    self._loss_box_classifier(
                        prediction_dict['refined_box_encodings'],
                        prediction_dict['class_predictions_with_background'],
                        prediction_dict['proposal_boxes'], prediction_dict[
        ...
        with tf.name_scope('RPNLoss'):
            (batch_cls_targets, batch_cls_weights, batch_reg_targets,
             batch_reg_weights, _) = target_assigner.batch_assign_targets(
                 ...
                 box_list.BoxList(anchors), groundtruth_boxlists,
    ...
    def _loss_box_classifier(self, refined_box_encodings,
                             class_predictions_with_background, proposal_boxes,
                             num_proposals, groundtruth_boxlists,
                             groundtruth_classes_with_background_list):
    ...
    """
        with tf.name_scope('BoxClassifierLoss'):
            ...
            second_stage_loc_losses = self._second_stage_localization_loss(
                ...
                batch_reg_targets,
                weights=batch_reg_weights) / normalizer
    ...
    def _unpad_proposals_and_apply_hard_mining(
            self, proposal_boxlists, second_stage_loc_losses,
            second_stage_cls_losses, num_proposals):
    ...
    """
        for (proposal_boxlist, single_image_loc_loss,
             single_image_cls_loss, single_image_num_proposals) in zip(
                 proposal_boxlists,
