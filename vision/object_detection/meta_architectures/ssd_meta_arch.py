...
class SSDMetaArch(model.DetectionModel):
    ...
    """
        ...
        with tf.name_scope('Preprocessor'):
            ...
            resized_inputs = tf.map_fn(
                self._image_resizer_fn, elems=inputs, dtype=tf.float32)
    ...
    """
        ...
        for idx, (feature_map, num_anchors_per_location) in enumerate(
                zip(feature_maps, num_anchors_per_location_list)):
            box_predictor_scope = 'BoxPredictor_{}'.format(idx)
            box_predictions = self._box_predictor.predict(
                feature_map, num_anchors_per_location, box_predictor_scope)
            ...
            if len(box_encodings_shape) != 4 or box_encodings_shape[2] != 1:
                raise RuntimeError(
                    'box_encodings from the box_predictor must be of '
        ...
        anchors_assert = tf.assert_equal(num_anchors, num_predictions, [
            'Mismatch: number of anchors vs number of predictions',
            num_anchors, num_predictions
        ])
        ...
        with tf.name_scope('Postprocessor'):
            ...
            (nmsed_boxes, nmsed_scores, nmsed_classes, _,
             num_detections) = self._non_max_suppression_fn(
                 detection_boxes, detection_scores, clip_window=clip_window)
            return {
